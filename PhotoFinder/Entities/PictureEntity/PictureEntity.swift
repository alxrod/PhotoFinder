...
class PictureEntity: Entity, HasAnchoring {
    ...
    init(image: UIImage, planePosition: SIMD3<Float>, planeSize: SIMD2<Float> = [0.25, 0.25]) {
        super.init()
...
//            cubeEntity = ModelEntity(mesh: cubeMesh, materials: [material])
//            rotator.addChild(cubeEntity!)
            
        }
//        rotator.position = SIMD3<Float>(0, 1, -2)
        rotator.position = planePosition
